...
#let letter-generic(
  ...
  set page(
    ...
    background: context {
      // Custom letterpaper background (if provided, only on first page)
      if letterpaper != none and counter(page).get().first() == 1 {
        letterpaper
      }
    ...
    footer-descent: 0%,
    footer: context {
      show: pad.with(top: 5pt, bottom: 5pt) // Normal

      let current-page = counter(page).get().first()
      let page-count = counter(page).final().first()

      // Measure custom footer height for debugging
...
#let letter-simple(
  ...
  // Set body text to IBM Plex Serif for CDU compliance
